...
function is_container_running() {
  ...
  if docker ps | grep -q "$_service_name"; then
#    echo "Container $_service_name em execução."
    return 0
  fi
#    echo "Container $_service_name não está em execução."
    return 1
}
...
#        </plugins>' "${PROJECT_DIR}/pom.xml"
    ...
    echo ">>> ${NO_COLOR}docker-compose build"
    docker-compose build

    echo_success "Projeto Maven gerado com sucesso em $PROJECT_DIR
    Acesse o diretório da aplicação gerada: cd $PROJECT_DIR
    A aplicação irá rodar no endereço \"http://localhost:8080/${ARTIFACT_ID}\"
    Execute o serviço tomcat para rodar a aplicação: \"docker-compose up\""
